...
\label{chap_problema}
Devido a enorme quantidade de sites disponiveis na internet, para o computador localizar estes sites na rede independemente de onde estes estejam armazenados, é nescessario a utilizaç~ao de servidores do tipo DNS. Estes servidores traduzem dominios www.redesfga.com em endereços de IP( seja ele IPv4 ou IPv6), assim tornando possivel a localizaç~ao dos servidores (ou conjunto de servidores) que hopedam o site requisitado. O DNS ser´a melhor detalhada no capitulo ~\ref{chap_dns}

Primeiramente introduzirmos o conceito de resolver DNS, estes são responsaveis por iniciar as consultas que ocasionarão na tradução do recurso procurado, por exemplo um a tradução de um nome de domínio em um endereço IP.

O DNSSEC possibilita uma maior segurança ao serviço DNS, permitindo assim que as consultas DNS efetuadas sejam autenticadas e validadas. Assim possibilitando uma proteção ao usuario em relação a diversos ataques referentes ao protocolo DNS, como: 
